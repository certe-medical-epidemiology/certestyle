...
At the bottom of each page after the title page, page numbers can be printed in the format `"x of y"`:
...
Note that the text `" of "` is language-specific. This text is set in the TeX template and only supports `lang: nl` (which prints `"x van y"`) and `lang: de` (which prints `"x von y"`). It will print `"x of y"` in all other cases.
