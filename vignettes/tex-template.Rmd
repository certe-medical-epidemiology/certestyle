...
Setting `lang` will make sure that all automated TeX texts are translated, such as "Table of Contents", "List of Figures", "Table", "Figure", etc. This setting accepts an [ISO-639-1 code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes).
...
# for R Markdown
date: "`r format2(Sys.Date(), 'd mmmm yyyy')`"
```
...
All colours can be set separately, greatly influencing the look and feel of the resulting document. All colours must be set in HTML-codes, with or without `#`, such as `#000000` or `000000` for black. Colour names (such as `"black"`) do not work.
...
At the bottom of each page after the title page, page numbers are given in the format `"x of y"`. The text `" of "` can be set in the YAML header, using `footer-page-delim`. Note that spaces before and after this text will be ignored by LaTeX.

```{yaml, eval = FALSE}
footer-page-delim: " van "
footer-page-delim: "van" # same
```

For other language-specific changes, set `lang` to an [ISO-639-1 code](https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes).
...
These example work in both R Markdown and Quarto. For each example, this is set:
...
This is the default; aside from logo's, no additional formatting has been defined:

```{yaml, eval = FALSE}
# ... the above part with title, etc. ...
titlepage-logo: "`r certestyle::rmarkdown_logo('certe')`"
footer-logo: "`r certestyle::rmarkdown_logo('footer')`"
```
...
colour-titlepage-full: "`r certestyle::colourpicker('certeroze3')`"
colour-subtitle: "`r certestyle::colourpicker('certeroze3')`"  # this effectively hides the text
colour-subtitle2: "`r certestyle::colourpicker('certeroze3')`" # this effectively hides the text
colour-verticalbars: "`r certestyle::colourpicker('certeroze3')`"
# all certeroze5, a very bright certeroze variant
colour-author-date: "`r certestyle::colourpicker('certeroze5')`"
colour-identifier: "`r certestyle::colourpicker('certeroze5')`"

# and the logos
titlepage-logo: "`r certestyle::rmarkdown_logo('certe-tekst-wit-links')`"
footer-logo: "`r certestyle::rmarkdown_logo('footer')`"
```
